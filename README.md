...
Then copy the Pyo wheels from your computer to this directory with (this command must be typed from your computer, not the Bela):
...
Cd to the Bela scripts directory with:

    cd /root/Bela/scripts

From the Bela/scripts directory, compile and run any of the examples in the 
...
without the ".py" extension). The second argument is the path of the pyo-bela/common folder,
which contains all needed files to run a bela project.
...
is set to point to the location of pyo-bela/common folder.
    ...
    export PYO_BELA_COMMON=/root/Bela/pyo-bela/common
